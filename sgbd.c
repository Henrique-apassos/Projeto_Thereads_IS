...
void *ler(void *arg){ //funcao de leitura
    ...
    int id = *(int*)arg; //recebimento do argumento (int)
    while (1) {
    // Espera até que a variavel de condicao seja sinalizada
    if (pthread_cond_wait(&estaEscrevendo, &mutexbasededados) != 0) {
        printf("Erro ao esperar a condição.\n");
        break;
    }

    // Depois que a condição é sinalizada, o thread pode acessar a base de dados
